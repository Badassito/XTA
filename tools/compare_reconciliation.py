...
_DEFAULT_POLICIES = ("union", "provenance", "cross_sections", "largest_island")
...
def resolve_saved_confidence(collection, run_manifest_path):
    ...
    for layer in predictions:
        ...
        files.extend(directory / name for name in ("metadata.json", "index.json", "scores.u8.zlib"))
...
def _confidence_summary(report):
    ...
    report["unavailable_standard_policies"] = [] if complete else ["confidence_voxel", "confidence_anchored", "baseline"]
...
def write_previews(directory: Path, methods: list[dict], shape, *, dataset_name: str) -> dict:
    """Show fixed central and max-union-area slices without selecting by accuracy."""
    ...
    union = np.memmap(union_record["_raw_path"], mode="r", dtype=np.uint8, shape=shape)
    areas = np.zeros(shape[0], np.int64)
    for z in range(shape[0]):
        areas[z] = np.count_nonzero(union[z])
    slices = [("Central slice", shape[0]//2), ("Largest raw-union slice", int(np.argmax(areas)))]
    ...
    try:
        for detail in (False, True):
            ...
            for row, method in enumerate(methods):
                current = np.memmap(method["_raw_path"], mode="r", dtype=np.uint8, shape=shape)
                try:
                    for column, (label, z) in enumerate(slices):
                        left = np.asarray(union[z])
                        right = np.asarray(current[z])
...
def compare(inputs, policy_paths, output, *, memory_mib=2048, previews=True, progress=_progress):
    ...
    report = {"schema": "xta.saved-reconciliation-comparison/1", "memory_mib": memory_mib,
              ...
              "unavailable_standard_policies": ["confidence_voxel", "confidence_anchored", "baseline"], "datasets": []}
    ...
    for dataset_index, manifest_path in enumerate(paths):
        ...
        with read_layer_manifest(manifest_path, workspace=work, memory_mib=max(1, float(memory_mib)/8), max_open=1024) as collection:
            ...
            confidence_readers, confidence_info, confidence_files = resolve_saved_confidence(collection, run_manifest_path)
            ...
            for policy_index, (settings, policy) in enumerate(policies):
                ...
                try:
                    result = reconcile(evidence, shape_tyx=shape, policy=policy,
                                       write_slab=lambda a,b,slab: array.__setitem__(slice(a,b), slab),
            ...
            for method in completed:
                Path(method.pop("_raw_path")).unlink()
            work.rmdir()
...
def build_parser():
    ...
    parser.add_argument("--policy", nargs="+", metavar="POLICY.py", default=None,
                        help="Defaults: union, provenance, cross_sections, largest_island. A raw-union reference is always included.")
