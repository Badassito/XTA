...
class CPUAugmentation:
    ...
    def _sample_parameters(seed: int, height: int, width: int) -> Dict[str, object]:
        ...
        rotation = float(rng.uniform(-45.0, 45.0))
        if rng.random() < 0.5:
            scale = float(rng.uniform(0.5, 1.0))
        else:
            scale = float(rng.uniform(1.0, 2.0))
        translate_x = float(rng.uniform(-0.1, 0.1) * width)
        translate_y = float(rng.uniform(-0.1, 0.1) * height)
        shear_x = float(rng.uniform(-30.0, 30.0))
        shear_y = float(rng.uniform(-30.0, 30.0))
        elastic = bool(rng.random() < 0.30)
        brightness = float(rng.uniform(0.8, 1.2)) if rng.random() < 0.50 else 1.0
        blur_sigma = float(rng.uniform(0.0, 5.0)) if rng.random() < 0.25 else 0.0
    ...
    def _apply_intensity_noise(
        ...
        *,
        ...
        if family == 0 and strength > 0.0:
            ...
            multiplier = generator.uniform(0.5, 1.5, size=sample.shape).astype(
                np.float32
            )
...
def build_augmentation() -> CPUAugmentation:
    """Build a seedable baseline policy for XTA's CPU augmentation backend."""
