"""Heavy CPU augmentation policy for PTA/TTA.
...
class CPUAugmentation:
    """Seedable OpenCV/NumPy implementation of the heavy probability graph."""
    ...
    def _sample_parameters(seed: int, height: int, width: int) -> Dict[str, object]:
        ...
        salt_pepper_amount = (
            float(rng.uniform(0.0, 0.065)) if rng.random() < 0.25 else 0.0
        )
        return {
    ...
    def _apply_intensity_noise(
        ...
        *,
        ...
        if family == 0 and strength > 0.0:
            ...
            multiplier = generator.uniform(0.35, 1.65, size=sample.shape).astype(
                np.float32
            )
...
def build_augmentation() -> CPUAugmentation:
    """Build a seedable heavy policy for XTA's CPU augmentation backend."""
